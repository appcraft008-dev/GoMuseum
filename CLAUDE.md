...
### 共享实体与 prod 数据操作（**最高优先级**，契约纪律 37；2026-09-26 跨馆作者事故）

- **共享实体只补缺、不覆盖。** 作者（`artists`，按 QID 全库共享、各馆共用一张作者卡）等被多件/多馆引用的行，
  单件/单馆命令（`generate --force/--qid/--limit`、`translate`、懒生成）**只许写缺失字段**。
  `--force` = 重做**这件作品**，永远不含共享实体；要刷新共享实体必须另写显式命令，先报「影响的馆×件数×作废音频数」给用户确认。
- **动 prod 数据前按代码列「写入面清单」**（会写哪些表→哪些行→还被谁引用），**不按命令名猜**；
  **备份范围 = 清单里的全部表**；先查「会作废多少条已有音频」，不为 0 就停下问用户。
- 兜底：每日 04:20 prod `pg_dump` 在 `/opt/gomuseum/backups/`，只按主键逐字段恢复受影响行，再独立比对。
- **音频 key 被清空、恢复完成前，禁止跑孤儿音频 GC**（会把可恢复事故变成永久丢失）。
- **机制已强制执行（纪律 37 ⑦）**：脚本进程里让已有音频失效的写入**默认被音频损失闸拦下**（`AudioLossBlocked`）；
  确需重录时用 `--allow-audio-loss N`（额度，不是开关），**不要为了"跑过去"而加大 N 或改闸**——先查为什么会丢音频。
  会话只用 `guarded_sessionmaker`/`SessionLocal`，不许直接 `sessionmaker(`，不许用批量语句删带音频的行。

### 遇到难题不轻易妥协（工作方式硬约束）

**"技术手段已穷尽"往往只是"我想到的手段已穷尽"。** 在提出降级方案（退回旧方案／接受瑕疵／砍需求）之前，必须先做完这三件事：

1. **换维度重新定义问题**——量化判据可能一开始就选错了。
   _实例_：调 TTS 音色时先后用「音高→亮度→低频→HNR」四个指标都没解决"丝丝声"，最后发现真正判据是**谐波锐度**；用错指标时所有优化都在南辕北辙（补低频反而把西语调成男声）。
2. **换组合方式，而不只是换参数**——同样的材料换个用法可能全解。
   _实例_：欧语语速快 44%，试遍降 cfg／插停顿／三种变速算法／调慢种子全失败；最后**用中文种子跨语言克隆**，语速和音质一次性同时解决。
3. **拿"已接受的基准"当靶心做全维度对标**，而不是自己拍阈值。
   _实例_：自定 HNR≥15 的门槛比 tts-1 自己（12-14.6）还严，误杀了合格候选；改成"按语言各自对标 tts-1 的 25 维指纹"后立刻选出正确种子。

**判断降级是否成立的标准**：能说清"这条路为什么物理上走不通"（如 Chatterbox 有 40 秒硬截断、官方 issue 十个月未修），而不是"我试了几种都不行"。
配套要求：失败的尝试**必须记录到配方文档**（见 `docs/architecture/tts-voice-production-recipe.md` §六），避免下次重复踩坑。
