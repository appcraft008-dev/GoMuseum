...
def upgrade() -> None:
    ...
    op.create_index(
        'ix_recognition_stats_id',
        'recognition_stats',
        ['id']
    )
    op.create_index(
        'ix_recognition_stats_date',
        'recognition_stats',
        ['date'],
        unique=True
    )
    ...
    op.create_check_constraint(
        'check_total_requests_positive',
        'recognition_stats',
        'total_requests >= 0'
    )
    op.create_check_constraint(
        'check_cache_hits_positive',
        'recognition_stats',
        'cache_hits >= 0'
    )
    op.create_check_constraint(
        'check_cache_misses_positive',
        'recognition_stats',
        'cache_misses >= 0'
    )
    op.create_check_constraint(
    ...
    op.create_index(
        'ix_ai_service_logs_id',
        'ai_service_logs',
        ['id']
    )
    op.create_index(
        'ix_ai_service_logs_recognition_id',
        'ai_service_logs',
        ['recognition_id']
    )
    op.create_index(
        'ix_ai_service_logs_timestamp',
        'ai_service_logs',
        ['timestamp']
    )
...
def downgrade() -> None:
    ...
    op.drop_constraint('check_p95_response_time_positive', 'recognition_stats', type_='check')
    op.drop_constraint('check_avg_response_time_positive', 'recognition_stats', type_='check')
    op.drop_constraint('check_cache_sum_equals_total', 'recognition_stats', type_='check')
    op.drop_constraint('check_cache_misses_positive', 'recognition_stats', type_='check')
    op.drop_constraint('check_cache_hits_positive', 'recognition_stats', type_='check')
    op.drop_constraint('check_total_requests_positive', 'recognition_stats', type_='check')
