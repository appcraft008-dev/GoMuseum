...
class TestRecognizeArtworkEndpoint:
    ...
    async def test_recognize_artwork_invalid_content_type(self):
        ...
        assert "Content-Type must be image/jpeg or image/png" in str(
    ...
    async def test_recognize_artwork_validation_exception(self):
        ...
        mock_service.recognize_artwork = AsyncMock(
            side_effect=ValidationException(
                "Image is corrupted", detail="Invalid JPEG format"
            )
        )
    ...
    async def test_recognize_artwork_timeout_exception(self):
        ...
        mock_service.recognize_artwork = AsyncMock(
            side_effect=TimeoutException(
                "Recognition timed out", detail="AI service took too long"
            )
        )
    ...
    async def test_recognize_artwork_service_exception(self):
        ...
        mock_service.recognize_artwork = AsyncMock(
            side_effect=ServiceException(
                "AI service unavailable", detail="OpenAI API down"
            )
        )
    ...
    async def test_recognize_artwork_logging(self, mock_logger):
        ...
        assert any(
            "Received recognition request for file: artwork.jpg" in msg
        ...
        assert any(
            f"Read {expected_len} bytes from uploaded file" in msg
            for msg in call_args_list
        )

        assert any(
            "Recognition successful: The Scream" in msg for msg in call_args_list
        )
