...
## 图标审计（2026-09-25，待办第 6 项，此前本轮没审图标）

对比图：`icon-comparison-48px.png`（从左到右：Smartify / TourBlink / **GoMuseum 现用** / **MOCK：底色改陶土红（仅示意，不是资产）** / MUSEUM BUDDY / Louvre Chatbot / TourLens；分别在 96px、48px 白底、48px 深色底）。竞品图标取自各自 Play 页面（2026-09-25），圆角遮罩为示意（Play 自己会加）。

**现用图标事实**（`docs/play-assets/icon_512.png`，2026-08-26 #391 刷新）：512×512 RGB 无透明通道、满版方形（符合 Play 要求，圆角由 Play 加）；底色奶油 `#F3EDDF`，柱廊建筑图案陶土红 `#A14E28`，四角有浅褐取景框角标 `#C2B69C`。

**实测数值**（WCAG 对比度）：

| 项 | 对比度 | 读法 |
|---|---|---|
| 图案 vs 底色 | 4.95 : 1 | 够，48px 下建筑仍能认出 |
| 底色 vs 白色 | **1.17 : 1** | ⚠️ 白底的搜索结果/详情页上，图标边缘几乎融进页面 |
| 取景框角标 vs 底色 | **1.72 : 1** | ⚠️ 48px 下基本看不见，「扫描」这个线索丢了 |

**判断（我的视觉判断，不是用户数据）**：
- ✅ **色相在竞品里是独一份**：竞品是白、绿、黑、蒙娜丽莎照片，只有我们是暖奶油+陶土红，列表里一眼能区分。
- ✅ 图案在 48px 下清楚，不含小字。
- ⚠️ **白底上边缘发虚**：Smartify 的近白底也有这个问题，但我们的奶油色更接近"页面底色"。深色模式下反而很突出。
- ⚠️ **只说"博物馆"，不说"扫描"**：唯一的扫描线索（角标）在 48px 下看不见；柱廊建筑本身也偏通用（像银行/法院）。而 Smartify 的 `ART` + 取景框角标在同尺寸下清楚，**我们与它共用"浅底 + 角标"的视觉语言但表现更弱**。
- 示意稿（陶土红底 + 奶油图案，同一素材重新着色，**角标仍然偏弱**）在白底和深底上都更突出，也与截图脚本里的强调色 `#A14E28` 同色系。

**Console 现状（用户 2026-09-25 截图确认）**：Play 里上传的就是上述奶油+柱廊版；用户目视同意"取景框过大、反差不明显"。

**桌面图标与商店图标其实不一致（已核对源码）**：Android 桌面用的是自适应图标——背景色 `#F3EDDF` + 前景 `assets/icon/app_icon_foreground.png`（**只有陶土红柱廊，没有取景框角标**）。角标只存在于商店图标（`app_icon.png` 1024 缩出的 `icon_512.png`）。所以**改商店图标的角标不需要发包，也不影响桌面图标**；只有改"底色/图案本身"才需要与桌面图标一起改并发包。

**候选稿**（`docs/play-assets/icon-candidates/`，**未上传、未改任何 App 资源**；对比图 `icon-candidates-48px.png`，含 48px 白底/深底）：
- **V1 奶油底 + 加粗角标**：角标改陶土红（与图案同色）、加粗到约 18/512、框收紧贴近柱廊。48px 下扫描线索清楚可见；底色不变→桌面图标无需改、**不需要发包**。白底边缘发虚的问题**仍在**。
- **V2 陶土红底 + 奶油图案与角标**：白底和深底上都最突出（解决边缘发虚），也最接近截图脚本的强调色；但底色变了→桌面图标应一并改、要发包。
- 试过的 V3（在柱廊上加一条扫描线）在 48px 下看不出来，已放弃。

**要不要改**：图标可以 A/B（Play 实验，最多 3 个变体，见第 1 节）。建议：**V1 可以直接换**（零风险、不发包、只修"看不见扫描"这一条）；V2 留到放量后与 V1 做 A/B，再决定是否连桌面图标一起换。以上均为我的视觉判断，不是用户数据。
