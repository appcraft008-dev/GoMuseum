...
`EN_08_footprints` 本轮不入选（留存类功能，弱转化）；备用图 `spare_pass_top`（EN 第 8 屏 / FR 第 7 屏 `PARIS CITY PASS` 的构图备选）、`spare_result_qa`（⚠️ 预设问答，文案必须写明非实时对话，不进前三张）。
...
| 8 | **素材是否为原图**（微信默认压缩，见下） | 🟡 **EN 10 张全为原图；FR 8 张里 7 张为原图，仅 `FR_06_indepth` 仍是压缩件**（JPEG 质量约 58）。实测其在商店页显示尺寸（≈490px）下 PSNR 42–45 dB、肉眼无差别，**可用，不阻塞**；若要「完美」，补拍 `En détail` 弹层（Artiste / Contexte / Analyse 几个 tab）的原图替换即可 |
| 9 | **成图导出规格** | 见下方「成图导出」 |

#### 成图导出规格（2026-09-25 实测得出）

- 画布 **1080 × 1920**（9:16）。原始截屏是 1080×2400（比例 2.22），**超过 Play 的 2:1 上限会被拒**，不能直接上传，必须重新合成。
- **导出用 PNG（24 位、无 alpha），或 JPEG 质量 ≥ 92。** 不要让设计工具默认导出低质量 JPEG：素材若是压缩件，再叠一次 Q75 导出会掉到 38 dB（可见），而 PNG / Q92 都在 42 dB 以上。这种版式的成图 PNG 约 0.5MB，远低于 Play 的 8MB 上限。
