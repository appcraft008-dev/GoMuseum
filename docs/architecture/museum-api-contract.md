...
⏳ **待实现(截至 2026-09-04 这道护栏还不存在,别以为已经有了)**:
让判定结论随文件一起交付并被灌入端核验 —— 现成载体是 `_state.json`
(已含每条的 ok/锐度/一致性,只差 md5),灌入端读 `--dir` 下的 `_state.json`,
对 `ok != true` 或 md5 不符的一律拒绝,失败方向是**拒绝**而不是放行。
⚠️ 在它落地之前,本条前半的"移出目录"是**唯一**的防线,而它挡不住拷错目录 ——
上面那起真实事故正是这道护栏缺失时发生的,不补上还会再来一次。
