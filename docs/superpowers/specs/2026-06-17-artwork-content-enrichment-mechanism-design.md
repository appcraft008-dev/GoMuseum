...
- **接地"源语言"≠ 生成"轴心语言"（关键区分）**：
  ...
  - **轴心生成语言（§7）保持单一**：默认英语、按馆可配。**不生成多份权威正文**（否则贵的"接地生成+蕴含校验"
    翻倍、且各份不一致无从翻译）。
- **馆开放数据（可插拔加分源，v1 不实现）**：Joconde/Europeana/Met/Rijksmuseum。沿用 v1 `Source` ABC +
  合并优先级，加连接器即接入、不改主流程。
...
- **软性考量（记一笔，非代码硬伤）**：① 轴心语言默认 en、**按馆可配**（非西方艺术若本国语言源/效果更优可改，
  §18b-1）；② **识别准确率跨类别未验证**——识别评测仅在绘画做过，雕塑/器物/文物等立体物识别率可能低，
  复制到非绘画馆时识别价值或打折、需重测（质量问题非代码）；③ **源更新后再生成**：Wikipedia 改了如何重生成留 v2
  （content_version 已为 prompt 升级铺路，但无"源变检测"）。
...
按依赖分期，每期独立交付、测试：
1. **多类别骨架（单一真相源配置 + 段落标签多语言）+ WikipediaSource（多源语言：en+馆国语言）+ 多类别抓取
   （图片可选、SPARQL 层 top-N/分页）+ 事实/标签补全（馆名/城市从 Wikidata 多语言 label）+ 礼貌抓取
   （UA/限速/退避/缓存/批量/熔断）**。
