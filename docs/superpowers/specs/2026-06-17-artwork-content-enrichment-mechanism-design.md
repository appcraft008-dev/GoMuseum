...
> ⚠️ **部分被取代（2026-06-22）**：抓取主干模型（§5/§17 Wikidata 单主干）、对象身份（qid 唯一）、
> 列表契约（§15/附录）已被 `2026-06-22-universal-catalog-spine-design.md`（通用全量目录主干 + stub 列表 + 懒生成）
> **取代/加法扩展**——CatalogSource 注册表替代 Wikidata 单主干、(拥有馆,馆藏号) 替代 qid 唯一、列表加 `content_status`、对象可为 stub。
> 识别接入（§12）已**拆为独立 brainstorm**（CLIP+GPT 兜底+墙签 OCR）。富化源（Wikipedia lead / Joconde 部分字段）将**加法提厚**。
> 本 spec 其余部分（生成/闸/翻译/编排/质量/契约加法原则）仍有效。详见新 spec 末节「Supersedes」表。
>
...
- **推荐问题 chips**：每件**预生成 0–4 个"问题+预回答"**（随 generate 阶段产出、grounded + 蕴含校验 + 多语言、
  落库）→ 秒出、免费、安全 + 内容钩子。点 chip 出预回答；继续追问走多轮 `/ask`。
  - **chip 质量标准（编码进 `build_qa_prompt`）**：① **绝不复述墙签硬事实**（标题/作者/年代/馆藏地/材质/尺寸/编号——已在页面，问之即噪音，头号红线）；
    ② **好奇心驱动**——游客站画前真会问的 why/how/who/故事（意义/争议/画中人/技法/典故/背景）；③ **接地 + 宁缺毋滥**（材料撑不起有趣问题就少出甚至不出，绝不用 trivia 凑数，故 0–4 条而非固定 3–4）；
    ④ **补充而非复述讲解**（从某段挑一根线让用户拉）；⑤ 答案 1–3 句、是钩子、留余味引导继续 `/ask`。质量上限受材料丰富度限制——prompt 是当下主杠杆，full fetch 富材料提升上限。
