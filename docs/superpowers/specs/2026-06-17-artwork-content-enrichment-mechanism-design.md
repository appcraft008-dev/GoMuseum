...
>
> ⚠️ **前端"呈现方式"仍在讨论中**：本 spec 定**数据契约**（后端提供什么）；前端**具体 UI/交互**
> 在前端那一期的计划里设计。判据：UX 怎么变都行，只要数据契约支撑得住就不动机制。明早继续逐个
> 过前端呈现、确认是否牵动后端契约。
...
- 前端分段藏品详情页 + 馆藏分类列表作为消费者，端到端闭环（含识别流）。

**非目标（留 v2+）**：
- Wikidata/Wikipedia 之外的馆开放数据连接器（Joconde/Europeana/Met）——接口预留，不实现。
...
- **前端具体 UI/交互**（类别 tab、详情 tab、连播、滚动——数据契约在此，UI 在前端期）。
...
- **Wikidata（骨架，多类别）**：提供结构化事实（作者/年代/材质/尺寸/藏地/depicts/sitelinks）
  ...
  奥赛含绘画/雕塑/摄影/装饰艺术等）。
- **Wikipedia 正文（叙事血肉）**：新增 `WikipediaSource`——按对象 sitelink 找对应 Wikipedia 页，
  拉**正文摘录**。**接地素材取最丰富语言源**（如奥赛法国艺术优先 fr.wikipedia + en）。质量天花板来源。
- **馆开放数据（可插拔加分源，v1 不实现）**：Joconde/Europeana/Met/Rijksmuseum。沿用 v1 `Source`
  ABC + 合并优先级，加连接器即接入、不改主流程。
- **源调用合规/限速（多馆规模化必需）**：Wikidata SPARQL/Wikimedia 有限额、要求**描述性 User-Agent**。
  连接器需**限速 + 重试 + 本地缓存**（同一对象的源拉取结果缓存复用），否则多馆批量会被限流/封。
...
- **机制第一步**：扩 `seed_sections.py` 为多类别，在 staging + prod 跑，让多类别 `object_content`
  能返回 tabs。生成器据对象类别取对应段落集。
...
2. **英语轴心生成**：一次 LLM 调用产出英语全部段落草稿（段落集**按对象类别**取；system prompt 强约束：
   只依据材料、**类别感知**（雕塑讲材质工艺/体量、摄影讲拍摄背景，泛化不硬编码单一类别）、
   **原创表达不照搬**、缺料留空段）。
...
识别流（拍照→识别）**必须接这套机制**，否则机制无意义。流程：
1. 识别返回**作品名 + 作者**（非 qid）→ **匹配到库内 qid**（名字/作者归一化匹配，**带置信门槛**：
   低于门槛宁可当未命中，也不**张冠李戴**返回错作品讲解）。
2. **命中且已预生成（top-N）→ 秒出**（读 DB/R2）。
3. **命中但属长尾未生成 → 懒生成**（§11② 渐进 UX，一次性）。
4. **未命中库（含用户随手拍的非藏品）→ 返回"未收录" + 反馈入口**；**后端记录识别出的标签文本**
...
## 15. 前端消费者：契约（此处）+ 呈现需求（前端期）

**入口**：馆藏列表点藏品 / 识别命中 → 详情页（现状 push `/guide` 实时重生成、浪费不落库，改向）。

**数据契约（属本 spec，必须对所有馆统一）**：
- **藏品列表端点（新增/改造，分页 + 类别筛 + 热门排序）**：现状 `/museums/{slug}` 一次性返回全部
  artworks（多类别跑全后上千条、巨型响应、今日崩于解析 246 条）。改为可分页、可按类别筛、按
  popularity 降序的列表端点（如 `/museums/{slug}/objects?category=&sort=popularity&limit=50&offset=0`）。
- **详情内容端点（已有）**：`GET .../objects/{qid}/content?language={当前语言}` → **按 sort_order 有序**
  分段 `{section_code,label,icon,body,audio_url,status}`；段 body 空/`needs_review` → "待完善"；
  对象 `absent` → `status: generating`（前端轮询直至 published）。

**呈现需求（记一笔，UI 在前端那一期计划设计；数据契约已支撑）**：
- 馆页**按类别 tab** 切换不同类别清单（数据：类别筛选端点）。
- 列表**热门排序 + 每次 50 条、上滑加载下 50**（数据：分页 + popularity 排序）。
- 详情页**分段 tab**（介绍/作者/背景/故事…），**每段一个语音播放键**（数据：有序段落 + 各段 audio_url）。
- **播完自动跳下一段 tab 并续播下一段语音**（纯前端：按 sort_order 顺序连播，后端零改动）。
- 播放器 0.75/1/1.25/1.5x 速率控件（`TtsService.setSpeed`，纯前端）。

> ⚠️ 明早继续逐个过前端呈现，确认是否还有牵动后端契约的点（如需要后端补字段/端点，再回这里改）。
...
**复制到新馆 = 配置 + 跑命令，无主流程改动**：
- **全局配置**：目标语言集（默认 `en,fr,de,es,it,zh`）、各类别段落集、品牌音色、模型档位、
  top-N 默认（painting=500,other=300）——馆无关、随时改。
- **每馆配置**（`museums.yaml`）：slug、Wikidata 馆 QID、抓取类别集、规模、语言/top-N 覆盖（可选）。
...
- 加新类别：全局配置加该类别段落集（无代码）。加新源：实现一个 `Source` 连接器接入。

## 19. 工程与合规约束（汇总）

- **版权（B）**：生成只取事实、原创表达，不照搬 Wikipedia 句式；App 内数据来源致谢。
- **源限速合规（C）**：连接器限速 + 重试 + 缓存 + 描述性 User-Agent；防多馆规模化被封。
- **图片版权（D，已知边界）**：详情页图依赖 Wikimedia 图（v1 强制 P18）。奥赛多 1950 前、公有领域，OK；
  **现代艺术馆**（蓬皮杜等）作品图常受版权 → 无图可显，复制时需知此边界。
...
按依赖分期，每期独立交付、测试：
1. **多类别骨架 + WikipediaSource + 多类别抓取 + 事实/标签补全 + 源限速缓存**。
...
5. **识别流接入**：识别→qid 匹配（置信门槛）+ 命中服务 + 未命中记录聚合。
6. **API：分页 + 类别筛 + 热门排序列表端点**。
7. **前端**：馆藏分类列表（类别 tab/分页/滚动）+ 分段详情页（tab/每段语音/自动连播/速率）+ 入口改向 +
   未收录提示。（UI 细节待明早前端呈现讨论后定计划。）
