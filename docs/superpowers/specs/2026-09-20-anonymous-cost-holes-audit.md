...
## 十三、staging 实测记录（2026-09-20，#609 合并后）

单测不够 —— 它测的是"我以为的接线"，实测才验证"部署出去的东西"。
以下全部打的是真实的 `staging-api.gomuseum.app`：

| # | 检查 | 结果 |
|---|---|---|
| 1 | `POST /content/explanation`（带 `qid=Q12418`） | **410** |
| 2 | `POST /content/tts/generate` section 模式（**无令牌**） | **410**（不是 401 —— 证明退役在权益闸**之前**） |
| 3 | 同上 ad-hoc 模式（无令牌） | **401**（付费墙原样健在 —— 退役没误伤） |
| 4 | `GET /recognition/{recent,stats,recognize/abc}` | **404 / 404 / 404** |
| 5 | `POST /recognition/recognize` | **410**（信号保留，没被误删成 404） |
| 6 | 匿名 `GET .../objects/Q12418/content` | **200**（正常读没被挡） |
| 7 | 带**坏令牌**打同一路径 | **200**（不是 401 —— 老 App 不会崩） |
| 8 | `GET /museums/louvre/objects?limit=99999` | `limit` 回显 **200**（截断生效，不是 422） |
| 9 | `GET /search?q=a&limit=99999` | 返回 **200** 条（同上） |

### 🔑 第 10 条才是真正的那一格：成对验证点火行为

前面 9 条里没有一条能证明"闸不是一刀切死"。拿一个真的 stub 件
（`orangerie/Q121160206`，从 staging DB 查出来的），**同一个 qid 打两次**：

| 调用者 | `generating` |
|---|---|
| 匿名 | **False** ← 不点火 |
| 持 `/auth/guest` 发的令牌 | **True** ← 点火 |

只做上面那一半的话，一个"把 `maybe_trigger` 整个删掉"的实现也会通过。

⚠️ 这条检查**真的花了约 $0.01**（它真跑了一次懒生成）—— 这是证明
"授权路径仍然工作"的唯一办法，单测里的桩证明不了部署出去的东西。
值这个钱，但要知道自己在花。
