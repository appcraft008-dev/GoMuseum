...
**规模触发换引擎**（写入本 spec，不拍脑袋）：藏品总量越过 ~5-10 万 / 实测查询延迟退化
（进程内线性扫描 + 内存 ×worker 数是瓶颈）/ 上第 N 个馆。届时换**自托管搜索引擎**
（Meilisearch / Typesense：CJK 分词好、子串/前缀/错字容忍、百万级即时搜索、自托管边际成本低——
同 DINOv2/Qwen3-TTS 理念）。换引擎只换 `search()` 实现，**契约与前端零改动**；唯一细微差别是
排序权重（引擎自有算法），"能搜到什么"完全一致。
