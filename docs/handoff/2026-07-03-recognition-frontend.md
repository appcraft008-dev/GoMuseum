...
## 补充（2026-07-04 用户测试反馈）：取景页加"从相册选图"入口

用户测试识别需要用下载的照片——取景页目前只有拍摄，无图库上传。加一个相册按钮
（`image_picker` 的 gallery source），选图后走与拍摄完全相同的提交链路（同端点同参数）。
后端无感（只收图片字节）。这也是正式用户的常见需求（帮别人拍的照片/相册里翻旧图）。

## ⚠️ 追加（2026-07-04 用户实测）：识别 401 与身份保险带

**现象**：服务端计费上线后,App 识别全部报"识别失败"——后端日志全是 401(令牌过期/失效且未恢复)。后端路径已验证健康(新 guest 令牌 curl 识别 200)。

**要修**：
1. 排查识别请求的 **401→AuthInterceptor 刷新→重试**链路——multipart FormData 请求的重试可能未生效(FormData 不可复用,重试需重建请求体)。
2. **保险带(强烈建议,一行)**：识别请求 queryParameters 永远附加 \`device_id\`(设备标识)——后端已支持 device_id 作为身份兜底(令牌失效时计费落到设备维度),识别功能对令牌抽风免疫。
3. 失败文案分档:401→"登录状态过期,请重试"(触发重新登录);402→付费墙引导;其余→"识别失败"。
