...
## ⚠️ 追加（2026-07-04 用户实测发现）：内容缓存必须区分完成态

**现象**：《阳光下的躯干》后端六语内容齐全、API 返回完整(status=ready, guide 301字, 3 tabs),但 App 显示"待完善+深度内容(1)"——**App 缓存了生成中途的半成品响应**,重开页面不再请求。

**规则**：content 响应**只有 `status == "ready"` 且 `generating == false` 才可落本地缓存**;
其它状态(生成中/待完善/资料不足)一律不缓存、每次打开重新请求。
已缓存条目建议打开时后台 revalidate(有更新则原地刷新),彻底杜绝半成品滞留。
