...
> **✅ 全部闭环（2026-07-04 后端回执）：**
> ① **已完成**（其实在交接提出前已由 #142 修复上线）——staging/prod 实测 `?language=it` → Dipinti/Sculture/Opere su carta。若仍见英文请刷新重测（前端测试时修复尚未部署）。
> ② **前提已失效，无需执行**——prod 讲解内容已按产品决定整体清空重来（保持空、按需懒生成），"243 件只有 zh/en/fr"的存量不复存在；现在**一切内容天生六语**。验收样本实测：Q3358957 `?language=de` → ready + 1626 字符德语讲解 ✅。批量 `translate` 命令已建好备用（未来任何补语种场景一条命令）。
> ③ **已完成**（#172）——`nationality_i18n`/`notable_works_i18n`（P27/P800 权威标签→翻译兜底→en），staging 实测：库尔贝 de=Frankreich+德语代表作、zh=法國+中文代表作、it=Francia ✅。存量 429 位作者已回填 426（尾巴幂等重跑收）。
> 待发 prod：③ 随下一次发布批次（含识别 P1）上线后重跑 `names` 即生效。
