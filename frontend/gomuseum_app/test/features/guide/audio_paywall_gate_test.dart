...
void main() {
  ...
  group('没买过的人 → 分档不变', () {
    ...
    // 🔴 这条原本断言「未认领 → 付费墙(免费名额从识别流程里发,不从浏览发)」,
    // 2026-09-19 真机推翻:
    //
    // ① 它让「保证送达的首体验」**永远到不了**。自动播的准入
    //    (audio_autoplay_test)判的是"未认领 → 播",但 _maybeAutoPlay 放行后
    //    立刻调 _onTap,而 _onTap 第一件事就是 _blockedByPaywall —— 也就是这里。
    //    两个文件各自手抄了一份判据、各自绿着,合起来是"识别完自动播,然后弹墙"。
    //    prod 日志实证:新账号识别成功后 `/audio` 请求**一条都没有**。
    // ② 后端对同一状态判的是 claimable(放行,送达后才认领),不看入口 ——
    //    入口区分本来也拦不住任何人(curl 就能拿到),只拦得住自己的用户。
    // ③ 用户侧的观感是"一次免费试听都没有,直接要买票",这是付费转化的反面。
    //
    // 「不从浏览发」那个意图由**自动播只在识别路径开启**来表达(camera_page 传
    // autoPlayAudio: true),而不是把主动点播的人挡在门外——他名额还在。
    test('未认领 → 放行(名额还没用掉,听成了才认领这一件)', () {
      expect(gateFor(free(), 'Q12418'), Gate.play);
    });
